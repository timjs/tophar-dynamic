...
We see that, at development time, we need to specify the number of \fun{vend} tasks that can be done at the same time.
...
To mitigate this problem in \TOPHAT, we extend it with \emph{task pools}.
...
Extensions of the normalisation relation of \TOPHAT\ is given in \cref{fig:semantics-dynamic-normalisation}.
Similar to the way normalisation works for parallel-and (as shown in \cref{fig:semantics-tophat}),
normalisation of task pools normalises each task in the pool and then constructs a new pool containing these normalised tasks.
...
\begin{example}[Wrong! Unwanted sharing when normalising template tasks]
  ...
  In case we would normalise the template task,
  ...
  \end{TASK}
  This is undesirable because it breaks our demand that every editor should have a \emph{unique} key.
  It would be unclear what to do with the input $k_1\Send 42$.
  Therefore, we delay normalisation of the template task until the moment they are added to the pool.
\end{example}
...
Then, we need to extend our failing, inputs, and value observation to deal with task pools.
\cref{fig:observations-dynamic} shows these additions.
The failing observation is $\False$ because
it is alwasy safe to step to task pools.
After all, one can always initialise a new task in the pool.
...
\begin{example}[Partial work in pools]
  ...
  Now we satisfy the guard in the definition of \fun{summarise} that the number of snacks should be four.
