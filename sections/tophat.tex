...
\begin{itemize}
  ...
  \item
    ...
    This layer contains two labelled small-step relations: handling ($\handle{\iota}$) and interaction ($\interact{\iota}$).
\end{itemize}
...
\todo{fix order of explenation}
An example of this, is assigning a value to a heap location, as seen in \seerule{N-Assign}.
\refrule{N-Pair} exemplifies a rule where each element of a combinator is normalised by threading state $\sigma$ through,
and the set of dirty references is calculated by taking the union.
