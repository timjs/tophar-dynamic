...
\newmacro{O-Hints-new}{
  \begin{function}
    \signature{         \Hints : \powerset{\mathrm{Normalised\ task} \times \mathrm{State} \times \mathrm{Input\ list} \times \mathrm{Path\ condition}} \times (\mathrm{Value} \to \mathrm{Path\ condition}) \to \powerset{\mathrm{Inputs}}} \\
    \Hints(ts, \psi) &=& \left\{
      \begin{array}{ll}
        hs & \ \when\ hs \neq \nothing \\
        \Hints(\big\{\uad
