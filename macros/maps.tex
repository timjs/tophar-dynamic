...
\newmacro{O-Generate}{
  \begin{function}
    ...
    \addlinespace
    \signature{         \Generate : \powerset{\mathrm{Normalised\ task} \times \mathrm{State} \times \mathrm{Input\ list} \times \mathrm{Path\ condition}} \times (\mathrm{Value} \to \mathrm{Path\ condition}) \to \powerset{\mathrm{Inputs}}} \\
    \Generate(ts, \psi) &=& \left\{
      \begin{array}{ll}
        hs & \ \when\ hs \neq \nothing \\
        \Generate(\big\{\uad
            (n',\sigma',\iota' \cdot I, \phi\land\phi') \uad
            \big| \uad
            ts \ni (n,\sigma,I,\phi) \quad
            n,\sigma\syminteract{\iota'}n'|_{\phi'},\sigma' \quad
      ...
      \right. \\
                     & & \where\ hs = \big\{ \uad
                           \iota \uad
                           \big| \uad
                           ts \ni (n,\sigma,\iota\cdot I,\phi) \quad
                           \Value(n,\sigma) = v \quad
                           \Sat(\psi(v) \land \phi) \uad
                         \big\}
  \end{function}
}
